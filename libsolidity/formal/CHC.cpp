...
void CHC::endVisit(ContractDefinition const& _contract)
{
	...
	{
		// In order not to have to recreate initializers for each contract for each inheritance hierarchy,
		// each initializer in the inheritance is called directly by the analyzed contract, that is,
		// the most derived contract.
		// However, when the constructors have arguments, the constructor call of a contract higher up in the inheritance
		// might not be at the most derived contract, so the symbolic argument equality chain is lost since those
		// variables are not part of all predicates.
		// The loop below re-adds the constructor parameter=argument equality chain for all base contracts.
		// This adds potentially useless constraints, but makes it possible not to have to recreate full
		// inheritance chains for every contract.
